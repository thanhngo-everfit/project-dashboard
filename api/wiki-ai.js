...
export default async function handler(req, res) {
  ...
  const sys = 'You are a technical writer who turns messy raw documents (pasted notes, exported docs, transcripts) '
    ...
    + 'Output the BODY using this exact lightweight markup (NOT full Markdown): '
    + '"## " for section headings, "### " for subsections, "#### " for minor headings; '
    + '"- " for bullets (indent two spaces per nesting level); **bold** for emphasis; `code` for inline code; '
    + 'tables as pipe-separated rows like "Column A | Column B | Column C" (one row per line, first row = header); '
    + 'plain URLs on their own (they auto-link). Do NOT use #, backtick fences, images, or HTML. '
